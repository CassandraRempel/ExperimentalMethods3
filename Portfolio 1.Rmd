...
```{r}
pacman::p_load(tidyverse,janitor, data.table, ggplot2)
```
...
```{r}
#load data into directory as seperate, they need to all work into github
#do I want a data frame or a tibble?

dfdemo <- read.csv("demo_train.csv")
dflength <- read.csv("LU_train.csv")
dftoken <- read.csv("token_train.csv")

#this forces the df into a tibble. the advantage of using a tibble is 
#as.tibble(dflength)
#as.tibble(dftoken)
#as.tibble (dfdemo)
...
str(dflength)
str(dftoken)
#cross analyze the data, so it makes sense
#change visit from int to factor 
#
...
#recode 1 to visit1. to homogeneize the data

dfdemo$Visit[dfdemo$Visit == 1] <- "visit1." 
dfdemo$Visit[dfdemo$Visit == 2] <- "visit2." 
dfdemo$Visit[dfdemo$Visit == 3] <- "visit3." 
dfdemo$Visit[dfdemo$Visit == 4] <- "visit4." 
dfdemo$Visit[dfdemo$Visit == 5] <- "visit5." 
dfdemo$Visit[dfdemo$Visit == 6] <- "visit6." 
...
#rename column  Child ID to SUBJ
colnames(dfdemo)[colnames(dfdemo)== "Child.ID"] <- "SUBJ"

#changes visit to factor
dfdemo$Visit <- as.factor (dfdemo$Visit)

#remove punctuation
str_replace(dfdemo$SUBJ, "[[:punct:]]", "")

str_replace(dflength$SUBJ, "[[:punct:]]", "")

str_replace(dftoken$SUBJ, "[[:punct:]]", "")

#rename Visit to VISIT
colnames(dfdemo)[colnames(dfdemo)== "Visit"] <- "VISIT"
...
```{r}
 #not quite sure what is specifically wanted here
#subset
...
#merge into a new dataset seperating visit 1 from the rest
# merge the dfs 
#need to solve, not quite it
mergedFile1 <- merge(dfdemo, dflength, by = "SUBJ")

mergedFile <- merge (mergedFile1, dftoken, by = "SUBJ")
...
```{r}
#only report scores from visit 1
#rename clinical variables ADOS and ADOS1
#need to get this working right and correct

Visit1df <- mergedFile, mergedFile$VISIT.x == "visit1."
...
```{r}
#annoymyze all participants
#change 1 to F and 2 to M (Female/Male)

mergedFile$Gender[mergedFile$Gender == 1] <- "F" 
mergedFile$Gender[mergedFile$Gender == 2] <- "M"

#changes subjects to 
mergedFile$SUBJ <- as.numeric (mergedFile$SUBJ)
mergedFile$SUBJ <- as.factor (mergedFile$SUBJ)
...
#kids who produced the most words 6th, shouldnt we compare the 1st vs the 6th

#kids who produced the least 1st
